...
def install_imghdr_fix():
    ...
    try:
        import imghdr
        print("âœ“ imghdr module already available, no fix needed!")
        return True
    except ImportError:
        ...
        try:
            with open(imghdr_path, 'w') as f:
                f.write(imghdr_code)
            print(f"âœ“ Created imghdr.py at: {imghdr_path}")
            
            # Test import
            import imghdr
            print("âœ“ imghdr module successfully installed!")
            print("\nYou can now run: python main.py")
            return True
            
        except Exception as e:
            print(f"âœ— Error creating imghdr.py: {e}")
            return False
